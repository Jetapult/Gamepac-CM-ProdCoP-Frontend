...
import { LetterText, Plus, Text, Upload } from "lucide-react";

/**
 * TimelineMarker Component
 * Renders a marker on the timeline to indicate the current frame position
 */
const TimelineMarker = React.memo(({ currentFrame, totalDuration }) => {
  ...
  const handleVideoUpload = (event) => {
    ...
    if (file) {
      ...
      const newClip = {
        ...
        start: 0, // Will be adjusted by addUserClip
        duration: 300, // Default duration, could be calculated from video metadata
        src: videoUrl,
        row: 0,
        fileName: file.name,
      };
      ...
      <div className="h-32 bg-gray-900 w-full overflow-hidden flex flex-col border border-gray-700 rounded-b-md">
        ...
        <div className="flex justify-between items-center border-b border-gray-700 p-4">
          <div className="flex items-center space-x-4">
             ...
             <label className="bg-gray-700 hover:bg-gray-600 text-white p-2 rounded-md transition-colors duration-200 flex items-center space-x-2 cursor-pointer">
              <input
                type="file"
                accept="video/*"
